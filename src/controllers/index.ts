...
import { ProductOptions } from "../Items/Product.interface";
...
export const getAllProducts = async (req: Request, res: Response) => {
  try {
    let { name, id, catList } = req.query;
    const options: ProductOptions = {}

    if(name) options.name = name as string
    if(id) options.id = id as string
    if(catList) options.categoriesId = catList as string

    const list = await getProducts(prisma, options)
    res.json(list)
